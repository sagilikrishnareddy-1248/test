...
from simulation_training import Simulation
...
if __name__ == "__main__":
    ...
    while episode < config['total_episodes']:
        print('\n----- Episode', str(episode+1),
              'of', str(config['total_episodes']))
        epsilon = 1.0 - (episode / config['total_episodes'])
        simulation_time, training_time, overall_accuracy = Simulation.run(
            episode, epsilon)
        ...
        if (episode == config['total_episodes']):
            ...
            print("overall_accuracy", sum(
                overall_accuracy)/(config['total_episodes']-3))
    ...
    Visualization.save_data_and_plot(
        data=Simulation.reward_store, filename='reward', xlabel='Episode', ylabel='Cumulative negative reward')
    Visualization.save_data_and_plot(data=Simulation.cumulative_wait_store,
                                     filename='delay', xlabel='Episode', ylabel='Cumulative delay (s)')
